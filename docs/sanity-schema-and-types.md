...
- **Block previews** — every page-builder block has a **per-block mockup preview** (`icon: makeBlockAddItemPreview(...)`, `components: { preview: <Block>Preview }` from `previews/blocks/<block-name>-preview.tsx` built on the `mockup.tsx` primitives, `preview: autoSelect([...])` — snippet and full convention in [page-builder-blocks.md](page-builder-blocks.md)). `SmartBlockPreview` is the bootstrap fallback while the mockup is being built. **Never add `preview.prepare()` to a block**: it conflicts with `components.preview`. Any field change must be reflected both in `autoSelect` and in the mockup component.
